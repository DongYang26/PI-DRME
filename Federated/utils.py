...
def save_map(name, setup_name, test_step_ests, test_step_gts, batchsize):
    ...
    test_step_ests = np.squeeze(np.array(test_step_ests), axis=0)
    test_step_gts = np.squeeze(np.array(test_step_gts), axis=0)
...
def send_message():
    ...
    resp = requests.post("https://www.autodl.com/api/v1/wechat/message/send",
                         json={
                             "title": "Federated has been finished",
                             "name": "Federated has been finished",
                         }, headers=headers)
