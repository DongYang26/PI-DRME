...
class FedAvgServer:  # used as a server
    def __init__(self, global_parameters, client_model, client_weights, _device):
        self.global_parameters = global_parameters
        self._device = _device
        self.client_weights = client_weights
        self.prev_models = [copy.deepcopy(global_parameters)
                            for i in range(len(client_weights))]
        self.local_parameters = [copy.deepcopy(
            client_model) for i in range(len(client_weights))]

    def upload(self, local_parameters):  # upload all user models to server
        self.local_parameters = copy.deepcopy(local_parameters)

        for key in self.global_parameters.state_dict().keys():
            for i in range(len(self.client_weights)):
                self.prev_models[i].state_dict()[key].data.copy_(
                    local_parameters[i].state_dict()[key])

            if 'num_batches_tracked' in key:
                self.global_parameters.state_dict()[key].data.copy_(
                    local_parameters.state_dict()[key])
            else:
                temp = torch.zeros_like(
                    self.global_parameters.state_dict()[key])
                for i in range(len(self.client_weights)):
                    temp += self.client_weights[i] * \
                        local_parameters[i].state_dict()[key]
                self.global_parameters.state_dict()[key].data.copy_(temp)

    def download(self, index):  # download client model from server
        clientModel = copy.deepcopy(self.local_parameters[index])
        for key in self.global_parameters.state_dict().keys():
            clientModel.state_dict()[key].data.copy_(
                self.global_parameters.state_dict()[key])
        return clientModel
...
class Client:  # as a user
    ...
    def train(self, model, prev_models, server_model, learningRate, idx, _device):  # training locally
        optimizer = torch.optim.Adam(model.parameters(), lr=learningRate)
        for local_epoch in range(0, local_epochs_s):
            for i, (inps, gts) in enumerate(self.data_loader):
                ...
                if local_epoch > 0 or i > 0:
                    posi = torch.tensor(0., device=_device)
                    nega = torch.tensor(0., device=_device)
                    for name, param in server_model.named_parameters():
                        curr_param = dict(model.named_parameters())[name]
                        posi += torch.norm(curr_param - param, p=2)

                    for prev_model in prev_models:
                        for name, _ in server_model.named_parameters():
                            prev_param = dict(
                                prev_model.named_parameters())[name]
                            curr_params = dict(model.named_parameters())[name]
                            nega += torch.norm(prev_param - curr_params, p=2)
                ...
                optimizer.step()
                print(
                    f"Client: {idx}({self.user_idx:2d}) Local Epoch S: [{local_epoch}][{i}/{len(self.data_loader)}]---- mseloss {loss.item():.4f}---- totalloss {total_loss.item():.4f}")
        for local_epoch in range(0, local_epochs_i):
            for i, (inps, gts) in enumerate(self.data_loader):
                inps = inps.float().to(_device)
                gts = gts.float().to(_device)
                optimizer.zero_grad()
...
def train(train_dataloaders, user_idxs, server, lr, _device):
    clients = activateClient(
        train_dataloaders, user_idxs)
    client_params = []
    for i in range(len(clients)):
        clientModel = server.download(i).to(_device)  # !
        prev_models = server.get_prev_models()
        for prev in prev_models:
            prev.eval()
        serv_model = server.get_global_models()
        serv_model.eval()
        clientModel.train()
        clients[i].train(clientModel, prev_models, serv_model, lr, i, _device)
...
def test(base_model, data_loaders, models, user_idx, _device):
    ...
    # save_map(f'{script_directory}/testVisualization/', setup_name,
